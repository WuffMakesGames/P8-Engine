...
_cst,_kwd,_api,_str,_dlm,_dgt,_hex,_sym=
split"true,false,nil,_𝘦𝘯𝘷,self"
...
function highlight(code)
	...
	while pos<len+1 do
		...
		elseif check"--" then
			...
			elseif get(char_at"0",_sym) then insert(symbol)
			-- digits
			else
				local data=check"0b" or check"0x"
				local check=check"0x" and _hex or _dgt
